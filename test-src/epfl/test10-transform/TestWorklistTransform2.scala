...
trait VectorExpTrans2 extends FWTransform2 with VectorExp with ArrayLoopsExp with ArrayMutationExp 
  ...
  override def onCreate[A:Typ](s: Sym[A], d: Def[A]) = (d match {
    ...
    case VectorApply(a,x) => s.atPhase(xform) { vapply_xform(xform(a), xform(x))(mtyp1[A]).asInstanceOf[Exp[A]] }
    case VectorLength(x)  => s.atPhase(xform) { vlength_xform(xform(x))(mtyp1[A]).asInstanceOf[Exp[A]] }
