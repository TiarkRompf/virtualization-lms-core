...
trait UncheckedOpsExp extends BaseExp {

  // TODO: use reifyEffects

  case class Unchecked[T](s: List[Any]) extends Def[T]
  def unchecked[T:Typ](s: Any*)(implicit pos: SourceContext): Rep[T] = reflectEffect[T](Unchecked(s.toList))
  def uncheckedPure[T:Typ](s: Any*)(implicit pos: SourceContext): Rep[T] = toAtom[T](Unchecked(s.toList))
