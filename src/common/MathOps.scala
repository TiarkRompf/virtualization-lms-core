...
trait ScalaGenMathOps extends BaseGenMathOps with ScalaGenEffect {
  ...
  override def emitNode(sym: Sym[Any], rhs: Def[Any]) = rhs match { // TODO: use java.lang.Math etc...
    case MathCeil(x) => emitValDef(sym, gen"java.lang.Math.ceil($x)")
    case MathFloor(x) => emitValDef(sym, gen"java.lang.Math.floor($x)")
    case MathExp(x) => emitValDef(sym, gen"java.lang.Math.exp($x)")
    case MathLog(x) => emitValDef(sym, gen"java.lang.Math.log($x)")
    case MathSqrt(x) => emitValDef(sym, gen"java.lang.Math.sqrt($x)")
    case MathSin(x) => emitValDef(sym, gen"java.lang.Math.sin($x)")
    case MathCos(x) => emitValDef(sym, gen"java.lang.Math.cos($x)")
    case MathAcos(x) => emitValDef(sym, gen"java.lang.Math.acos($x)")
    case MathAtan(x) => emitValDef(sym, gen"java.lang.Math.atan($x)")
    case MathAtan2(x,y) => emitValDef(sym, gen"java.lang.Math.atan2($x, $y)")
    case MathPow(x,y) => emitValDef(sym, gen"java.lang.Math.pow($x,$y)")
    case MathAbs(x) => emitValDef(sym, gen"java.lang.Math.abs($x)")
    case MathMax(x,y) => emitValDef(sym, gen"java.lang.Math.max($x, $y)")
    case MathMin(x,y) => emitValDef(sym, gen"java.lang.Math.min($x, $y)")
...
trait ScalaGenMathOpsApacheCommons extends ScalaGenMathOps {
  ...
  override def emitNode(sym: Sym[Any], rhs: Def[Any]) = rhs match { // TODO: use java.lang.Math etc...
    case MathCeil(x) => emitValDef(sym, gen"org.apache.commons.math.util.FastMath.ceil($x)")
    case MathFloor(x) => emitValDef(sym, gen"org.apache.commons.math.util.FastMath.floor($x)")
    case MathExp(x) => emitValDef(sym, gen"org.apache.commons.math.util.FastMath.exp($x)")
    case MathLog(x) => emitValDef(sym, gen"org.apache.commons.math.util.FastMath.log($x)")
    case MathSqrt(x) => emitValDef(sym, gen"org.apache.commons.math.util.FastMath.sqrt($x)")
    case MathSin(x) => emitValDef(sym, gen"org.apache.commons.math.util.FastMath.sin($x)")
    case MathCos(x) => emitValDef(sym, gen"org.apache.commons.math.util.FastMath.cos($x)")
    case MathAcos(x) => emitValDef(sym, gen"org.apache.commons.math.util.FastMath.acos($x)")
    case MathAtan(x) => emitValDef(sym, gen"org.apache.commons.math.util.FastMath.atan($x)")
    case MathAtan2(x,y) => emitValDef(sym, gen"org.apache.commons.math.util.FastMath.atan2($x, $y)")
    case MathPow(x,y) => emitValDef(sym, gen"org.apache.commons.math.util.FastMath.pow($x,$y)")
    case MathAbs(x) => emitValDef(sym, gen"org.apache.commons.math.util.FastMath.abs($x)")
    case MathMax(x,y) => emitValDef(sym, gen"org.apache.commons.math.util.FastMath.max($x, $y)")
    case MathMin(x,y) => emitValDef(sym, gen"org.apache.commons.math.util.FastMath.min($x, $y)")
...
trait CudaGenMathOps extends BaseGenMathOps with CudaGenEffect {
  ...
  override def emitNode(sym: Sym[Any], rhs: Def[Any]) = rhs match {
    case MathCeil(x) => emitValDef(sym, gen"ceil($x)")
    case MathFloor(x) => emitValDef(sym, gen"floor($x)")
    case MathExp(x) => emitValDef(sym, gen"exp($x)")
    case MathLog(x) => emitValDef(sym, gen"log($x)")
    case MathSqrt(x) => emitValDef(sym, gen"sqrt($x)")
    case MathSin(x) => emitValDef(sym, gen"sin($x)")
    case MathCos(x) => emitValDef(sym, gen"cos($x)")
    case MathAcos(x) => emitValDef(sym, gen"acos($x)")
    case MathAtan(x) => emitValDef(sym, gen"atan($x)")
    case MathAtan2(x,y) => emitValDef(sym, gen"atan2($x, $y)")
    case MathPow(x,y) => emitValDef(sym, gen"pow($x,$y)")
    case MathAbs(x) => emitValDef(sym, gen"fabs($x)")
    case MathMax(x,y) if(remap(sym.tp))=="float" => emitValDef(sym, gen"fmax($x, $y)")
    case MathMax(x,y) if(remap(sym.tp))=="double" => emitValDef(sym, gen"fmax($x, $y)")
    case MathMax(x,y) if(remap(sym.tp))=="int" => emitValDef(sym, gen"max($x, $y)")
    case MathMin(x,y) if(remap(sym.tp))=="float" => emitValDef(sym, gen"fmin($x, $y)")
    case MathMin(x,y) if(remap(sym.tp))=="double" => emitValDef(sym, gen"fmin($x, $y)")
    case MathMin(x,y) if(remap(sym.tp))=="int" => emitValDef(sym, gen"min($x, $y)")
...
trait OpenCLGenMathOps extends BaseGenMathOps with OpenCLGenEffect {
  ...
  override def emitNode(sym: Sym[Any], rhs: Def[Any]) = rhs match {
    ...
    case MathLog(x) => emitValDef(sym, gen"log($x)")
    case MathSqrt(x) => emitValDef(sym, gen"sqrt($x)")
    case MathAbs(x) => emitValDef(sym, gen"abs($x)")
    case MathMax(x,y) => emitValDef(sym, gen"max($x, $y)")
    case MathMin(x,y) => emitValDef(sym, gen"min($x, $y)")
    case _ => super.emitNode(sym, rhs)
  }
}
...
trait CGenMathOps extends BaseGenMathOps with CGenEffect {
  ...
  override def emitNode(sym: Sym[Any], rhs: Def[Any]) = rhs match {
    case MathSin(x) if(remap(sym.tp)=="double") => emitValDef(sym, gen"sin($x)")
