...
trait Equal extends Base with Variables with OverloadHack {
  ...
  def __ext__!=[A,B](a: Rep[A], b: Rep[B]) : Rep[Boolean]
  def __ext__!=[A,B](a: Rep[A], b: Var[B])(implicit o: Overloaded1) : Rep[Boolean] = __ext__!=(a, varToRep(b))
  def __ext__!=[A,B](a: Var[A], b: Rep[B])(implicit o: Overloaded2) : Rep[Boolean] = __ext__!=(varToRep(a), b)
...
trait EqualExp extends Equal with BaseExp with VariablesExp {
  ...
  def __ext__!=[A,B](a: Rep[A], b: Rep[B]): Rep[Boolean] = NotEqual(a,b)
}
