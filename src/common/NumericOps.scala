...
trait ScalaGenNumericOps extends ScalaGenFat {
  ...
  override def emitNode(sym: Sym[Any], rhs: Def[Any]) = rhs match {
    case NumericPlus(a,b) => emitValDef(sym, raw"${quote(a)} + ${quote(b)}")
    case NumericMinus(a,b) => emitValDef(sym, raw"${quote(a)} - ${quote(b)}")
    case NumericTimes(a,b) => emitValDef(sym, raw"${quote(a)} * ${quote(b)}")
    case NumericDivide(a,b) => emitValDef(sym, raw"${quote(a)} / ${quote(b)}")
