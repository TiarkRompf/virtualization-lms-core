...
trait ListOps extends Variables {
  ...
  implicit def varToListOps[T:Typ](x: Var[List[T]])(implicit pos: SourceContext) = new ListOpsCls(readVar(x)) // FIXME: dep on var is not nice
  implicit def repToListOps[T:Typ](a: Rep[List[T]])(implicit pos: SourceContext) = new ListOpsCls(a)
  implicit def listToListOps[T:Typ](a: List[T])(implicit pos: SourceContext) = new ListOpsCls(unit(a))

  class ListOpsCls[A:Typ](l: Rep[List[A]])(implicit pos: SourceContext) {
...
trait ListOpsExp extends ListOps with BaseExp with VariablesExp with BooleanOpsExp with ArrayOpsExp with StringOpsExp {
  implicit def listTyp[T:Typ]: Typ[List[T]] = {
    implicit val ManifestTyp(m) = typ[T]
    manifestTyp
  }
  case class ListNew[A:Typ](xs: Seq[Rep[A]]) extends Def2[A, List[A]]
  ...
  def list_new[A:Typ](xs: Seq[Rep[A]])(implicit pos: SourceContext) = toAtom(ListNew(xs))
  def list_fromseq[A:Typ](xs: Rep[Seq[A]])(implicit pos: SourceContext) = toAtom(ListFromSeq(xs))
  ...
  def list_toarray[A:Typ](l: Exp[List[A]])(implicit pos: SourceContext) = toAtom(ListToArray(l))
  def list_toseq[A:Typ](l: Exp[List[A]])(implicit pos: SourceContext) = toAtom(ListToSeq(l))
  def list_prepend[A:Typ](l: Exp[List[A]], e: Exp[A])(implicit pos: SourceContext) = toAtom(ListPrepend(l,e))
  def list_concat[A:Typ](xs: Rep[List[A]], ys: Rep[List[A]])(implicit pos: SourceContext) = toAtom(ListConcat(xs,ys))
  def list_cons[A:Typ](x: Rep[A], xs: Rep[List[A]])(implicit pos: SourceContext) = toAtom(ListCons(x,xs))
  def list_mkString[A:Typ](l: Exp[List[A]])(implicit pos: SourceContext) = toAtom(ListMkString(l))
  def list_mkString2[A:Typ](l: Rep[List[A]], sep:Rep[String])(implicit pos: SourceContext) = toAtom(ListMkString2(l,sep))
  def list_head[A:Typ](xs: Rep[List[A]])(implicit pos: SourceContext) = toAtom(ListHead(xs))
  def list_tail[A:Typ](xs: Rep[List[A]])(implicit pos: SourceContext) = toAtom(ListTail(xs))
  def list_isEmpty[A:Typ](xs: Rep[List[A]])(implicit pos: SourceContext) = toAtom(ListIsEmpty(xs))
...
trait ListOpsExpOpt extends ListOpsExp {
  override def list_concat[A : Typ](xs1: Exp[List[A]], xs2: Exp[List[A]])(implicit pos: SourceContext): Exp[List[A]] = (xs1, xs2) match {
    case (Def(ListNew(xs1)), Def(ListNew(xs2))) => toAtom(ListNew(xs1 ++ xs2))
...
trait BaseGenListOps extends GenericCodegen {
  val IR: ListOpsExp
  import IR._

}

trait ScalaGenListOps extends BaseGenListOps with ScalaGenBase {
  val IR: ListOpsExp
  import IR._
...
trait CudaGenListOps extends CudaGenBase with CLikeGenListOps
trait OpenCLGenListOps extends OpenCLGenBase with CLikeGenListOps
trait CGenListOps extends CGenBase with CLikeGenListOps
