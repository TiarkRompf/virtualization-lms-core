  ...
  def transformAll(scope: Seq[Stm], t: SubstTransformer): Seq[Stm] = {
    ...
    scope flatMap {
      ...
      case TTP(lhs, mhs, SimpleFatIfThenElse(c,as,bs)) =>
        ...
        def extractIf(d: Def[Any]) = d match {
          case l: AbstractIfThenElse[_] => l
          case Reflect(l: AbstractIfThenElse[_], _, _) => l
        }
        val cond2 = if (lhs != lhs2) mhs2.map (extractIf(_).cond) reduceLeft { (s1,s2) => assert(s1==s2,"conditions don't agree: "+s1+","+s2); s1 }
                    else t(c)
        val as2 = (if (lhs != lhs2) (lhs2 zip (mhs2 map (extractIf(_).thenp)))
                   else (lhs zip as)) map { case (s,r) => transformIfBody(s,r,t) }
        val bs2 = (if (lhs != lhs2) (lhs2 zip (mhs2 map (extractIf(_).elsep)))
