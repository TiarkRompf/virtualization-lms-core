...
trait ScalaGenPrimitiveOps extends ScalaGenBase {
  ...
  override def emitNode(sym: Sym[Any], rhs: Def[Any]) = rhs match {
    case ObjDoubleParseDouble(s) => emitValDef(sym, raw"java.lang.Double.parseDouble(${quote(s)})")
    ...
    case DoubleFloatValue(lhs) => emitValDef(sym, raw"${quote(lhs)}.floatValue()")
    case ObjIntegerParseInt(s) => emitValDef(sym, raw"java.lang.Integer.parseInt(${quote(s)})")
    case ObjIntMaxValue() => emitValDef(sym, "scala.Int.MaxValue")
    case ObjIntMinValue() => emitValDef(sym, "scala.Int.MinValue")
    case IntDivideFrac(lhs,rhs) => emitValDef(sym, raw"${quote(lhs)} / ${quote(rhs)}")
    case IntDivide(lhs,rhs) => emitValDef(sym, raw"${quote(lhs)} / ${quote(rhs)}")
    case IntMod(lhs,rhs) => emitValDef(sym, raw"${quote(lhs)} % ${quote(rhs)}")
    case IntBinaryOr(lhs,rhs) => emitValDef(sym, raw"${quote(lhs)} | ${quote(rhs)}")
    case IntBinaryAnd(lhs,rhs) => emitValDef(sym, raw"${quote(lhs)} & ${quote(rhs)}")
    case IntBinaryXor(lhs,rhs) => emitValDef(sym, raw"${quote(lhs)} ^ ${quote(rhs)}")
    case IntDoubleValue(lhs) => emitValDef(sym, raw"${quote(lhs)}.doubleValue()")
    case IntFloatValue(lhs) => emitValDef(sym, raw"${quote(lhs)}.floatValue()")
    case IntBitwiseNot(lhs) => emitValDef(sym, raw"~${quote(lhs)}")
    case IntToLong(lhs) => emitValDef(sym, raw"${quote(lhs)}.toLong")
    case LongBinaryOr(lhs,rhs) => emitValDef(sym, raw"${quote(lhs)} | ${quote(rhs)}")
    case LongBinaryAnd(lhs,rhs) => emitValDef(sym, raw"${quote(lhs)} & ${quote(rhs)}")   
    case LongShiftLeft(lhs,rhs) => emitValDef(sym, raw"${quote(lhs)} << ${quote(rhs)}")
    case LongShiftRightUnsigned(lhs,rhs) => emitValDef(sym, raw"${quote(lhs)} >>> ${quote(rhs)}")   
    case LongToInt(lhs) => emitValDef(sym, raw"${quote(lhs)}.toInt")
...
trait CLikeGenPrimitiveOps extends CLikeGenBase {
  ...
  override def emitNode(sym: Sym[Any], rhs: Def[Any]) = {
    rhs match {
      ...
      //case ObjDoubleMinValue() => emitValDef(sym, "scala.Double.MinValue")
      case DoubleFloatValue(lhs) => emitValDef(sym, raw"(float)${quote(lhs)}")
      ...
      case IntBinaryOr(lhs,rhs) => emitValDef(sym, raw"${quote(lhs)} | ${quote(rhs)}")
      case IntBinaryAnd(lhs,rhs) => emitValDef(sym, raw"${quote(lhs)} & ${quote(rhs)}")
      case IntDoubleValue(lhs) => emitValDef(sym, raw"(double)${quote(lhs)}")
      case _ => super.emitNode(sym, rhs)
    }
  }
}
