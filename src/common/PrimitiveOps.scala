...
trait ScalaGenPrimitiveOps extends ScalaGenBase {
  ...
  override def emitNode(sym: Sym[Any], rhs: Def[Any]) = rhs match {
    case ObjDoubleParseDouble(s) => emitValDef(sym, gen"java.lang.Double.parseDouble($s)")
    ...
    case DoubleFloatValue(lhs) => emitValDef(sym, gen"$lhs.floatValue()")
    case ObjIntegerParseInt(s) => emitValDef(sym, gen"java.lang.Integer.parseInt($s)")
    case ObjIntMaxValue() => emitValDef(sym, "scala.Int.MaxValue")
    case ObjIntMinValue() => emitValDef(sym, "scala.Int.MinValue")
    case IntDivideFrac(lhs,rhs) => emitValDef(sym, gen"$lhs / $rhs")
    case IntDivide(lhs,rhs) => emitValDef(sym, gen"$lhs / $rhs")
    case IntMod(lhs,rhs) => emitValDef(sym, gen"$lhs % $rhs")
    case IntBinaryOr(lhs,rhs) => emitValDef(sym, gen"$lhs | $rhs")
    case IntBinaryAnd(lhs,rhs) => emitValDef(sym, gen"$lhs & $rhs")
    case IntBinaryXor(lhs,rhs) => emitValDef(sym, gen"$lhs ^ $rhs")
    case IntDoubleValue(lhs) => emitValDef(sym, gen"$lhs.doubleValue()")
    case IntFloatValue(lhs) => emitValDef(sym, gen"$lhs.floatValue()")
    case IntBitwiseNot(lhs) => emitValDef(sym, gen"~$lhs")
    case IntToLong(lhs) => emitValDef(sym, gen"$lhs.toLong")
    case LongBinaryOr(lhs,rhs) => emitValDef(sym, gen"$lhs | $rhs")
    case LongBinaryAnd(lhs,rhs) => emitValDef(sym, gen"$lhs & $rhs")   
    case LongShiftLeft(lhs,rhs) => emitValDef(sym, gen"$lhs << $rhs")
    case LongShiftRightUnsigned(lhs,rhs) => emitValDef(sym, gen"$lhs >>> $rhs")   
    case LongToInt(lhs) => emitValDef(sym, gen"$lhs.toInt")
...
trait CLikeGenPrimitiveOps extends CLikeGenBase {
  ...
  override def emitNode(sym: Sym[Any], rhs: Def[Any]) = {
    rhs match {
      ...
      //case ObjDoubleMinValue() => emitValDef(sym, "scala.Double.MinValue")
      case DoubleFloatValue(lhs) => emitValDef(sym, gen"(float)$lhs")
      ...
      case IntBinaryOr(lhs,rhs) => emitValDef(sym, gen"$lhs | $rhs")
      case IntBinaryAnd(lhs,rhs) => emitValDef(sym, gen"$lhs & $rhs")
      case IntDoubleValue(lhs) => emitValDef(sym, gen"(double)$lhs")
      case _ => super.emitNode(sym, rhs)
    }
  }
}
