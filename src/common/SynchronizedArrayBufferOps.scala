...
trait ScalaGenSynchronizedArrayBufferOps extends BaseGenSynchronizedArrayBufferOps with ScalaGenArrayBufferOps {
  ...
  override def emitNode(sym: Sym[Any], rhs: Def[Any]) = rhs match {
    case a@SyncArrayBufferNew(xs) => emitValDef(sym, raw"(new scala.collection.mutable.ArrayBuffer[${remap(a.mA)}] with scala.collection.mutable.SynchronizedBuffer[${remap(a.mA)}]) ++= List(${(xs map {quote}).mkString(",")})")
