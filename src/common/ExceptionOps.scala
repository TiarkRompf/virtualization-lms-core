...
trait ExceptionOpsExp extends ExceptionOps with StringOpsExp {
  case class ThrowException(m: Rep[String]) extends Def[Unit]

  def throw_exception(m: Exp[String])(implicit pos: SourceContext) = reflectEffect(ThrowException(m), Global())

  override def mirror[A:Typ](e: Def[A], f: Transformer)(implicit pos: SourceContext): Exp[A] = (e match {
    case Reflect(ThrowException(s), u, es) => reflectMirrored(Reflect(ThrowException(f(s)), mapOver(f,u), f(es)))(mtype(typ[A]), pos)
