...
trait FractionalOps extends ImplicitOps {
  def infix_/[A,T](lhs: Rep[T], rhs: Rep[A])(implicit c: A => T, f: Fractional[T], mA: Manifest[A], mT: Manifest[T], ctx: SourceContext) = fractional_divide(lhs,implicit_convert[A,T](rhs))

  def fractional_divide[T:Fractional:Manifest](lhs: Rep[T], rhs: Rep[T])(implicit ctx: SourceContext): Rep[T]
}
...
trait FractionalOpsExp extends FractionalOps with ImplicitOpsExp {
  ...
  def fractional_divide[T:Fractional:Manifest](lhs: Exp[T], rhs: Exp[T])(implicit ctx: SourceContext) : Rep[T] = FractionalDivide(lhs, rhs)
}
