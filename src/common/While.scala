...
trait While extends Base {
  def __whileDo[A:Manifest](cond: => Rep[Boolean], body: => Rep[A])(implicit pos: SourceContext): Rep[Unit]
}


trait WhileExp extends While with EffectExp {
  case class While[A:Manifest](cond: Block[Boolean], body: Block[A]) extends Def[Unit]

  override def __whileDo[A:Manifest](cond: => Exp[Boolean], body: => Rep[A])(implicit pos: SourceContext) = {
