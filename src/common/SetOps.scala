...
import scala.lms.codegen.{GenericCodegen,CLikeCodegen}
...
trait SetOpsExp extends SetOps with ArrayOps with BooleanOps with BaseExp {
  implicit def setTyp[T:Typ]: Typ[Set[T]] = {
  ...
  case class SetToArray[A:Typ](s: Exp[Set[A]])(implicit pos: SourceContext) extends Def[Array[A]] {
    val array = NewArray[A](set_size(s))
  }

  def set_new[A:Typ](xs: Seq[Exp[A]])(implicit pos: SourceContext) = reflectMutable(SetNew(xs, typ[A]))
  def set_contains[A:Typ](s: Exp[Set[A]], i: Exp[A])(implicit pos: SourceContext) = toAtom(SetContains(s, i))
  ...
  def set_size[A:Typ](s: Exp[Set[A]])(implicit pos: SourceContext) = toAtom(SetSize(s))
  def set_clear[A:Typ](s: Exp[Set[A]])(implicit pos: SourceContext) = reflectWrite(s)(SetClear(s))
  def set_toseq[A:Typ](s: Exp[Set[A]])(implicit pos: SourceContext) = toAtom(SetToSeq(s))
  def set_toarray[A:Typ](s: Exp[Set[A]])(implicit pos: SourceContext) = toAtom(SetToArray(s))
}

trait BaseGenSetOps extends GenericCodegen {
  val IR: SetOpsExp
  import IR._

}

trait ScalaGenSetOps extends BaseGenSetOps with ScalaGenBase {
  val IR: SetOpsExp
  import IR._
...
trait CudaGenSetOps extends CudaGenBase with CLikeGenSetOps
trait OpenCLGenSetOps extends OpenCLGenBase with CLikeGenSetOps
trait CGenSetOps extends CGenBase with CLikeGenSetOps
