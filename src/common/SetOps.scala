...
trait ScalaGenSetOps extends BaseGenSetOps with ScalaGenEffect {
  ...
  override def emitNode(sym: Sym[Any], rhs: Def[Any]) = rhs match {
    case SetNew(xs, mA) => emitValDef(sym, raw"collection.mutable.HashSet[${remap(mA)}](" + (xs map {quote}).mkString(",") + ")")
    case SetContains(s,i) => emitValDef(sym, raw"${quote(s)}.contains(${quote(i)})")
    case SetAdd(s,i) => emitValDef(sym, raw"${quote(s)}.add(${quote(i)})")
    case SetRemove(s,i) => emitValDef(sym, raw"${quote(s)}.remove(${quote(i)})")
    case SetSize(s) => emitValDef(sym, raw"${quote(s)}.size")
    case SetClear(s) => emitValDef(sym, raw"${quote(s)}.clear()")
    case SetToSeq(s) => emitValDef(sym, raw"${quote(s)}.toSeq")
    case n@SetToArray(s) => //emitValDef(sym, quote(s) + ".toArray")
      ...
      stream.println(raw"val in = ${quote(s)}.toSeq")
