...
trait ScalaGenObjectOps extends ScalaGenBase {
  ...
  override def emitNode(sym: Sym[Any], rhs: Def[Any]) = rhs match {
    case ObjectToString(lhs) => emitValDef(sym, raw"(${quote(lhs)}).toString()")
    case ObjectUnsafeImmutable(x) => emitValDef(sym, raw"${quote(x)}// unsafe immutable")
    case ObjectUnsafeMutable(x) => emitValDef(sym, raw"${quote(x)}// unsafe mutable")
