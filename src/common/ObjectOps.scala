...
trait ObjectOpsExp extends ObjectOps with StringOpsExp with VariablesExp {
  ...
  case class ObjectUnsafeImmutable[A:Typ](o: Exp[A]) extends Def1[A]
  case class ObjectUnsafeMutable[A:Typ](o: Exp[A]) extends Def1[A]

  def object_tostring(lhs: Exp[Any])(implicit pos: SourceContext) = toAtom(ObjectToString(lhs))
  def object_unsafe_immutable[A:Typ](lhs: Exp[A])(implicit pos: SourceContext) = lhs match {
    ...
    case c@Const(x) => c
    case _ => toAtom(ObjectUnsafeImmutable(lhs))
  }
  ...
  override def mirror[A:Typ](e: Def[A], f: Transformer)(implicit pos: SourceContext): Exp[A] = (e match {
    case e@ObjectUnsafeImmutable(a) => object_unsafe_immutable(f(a))(mtype(e.mR),pos)
    case e@ObjectToString(a) => object_tostring(f(a))
    case Reflect(e@ObjectUnsafeImmutable(a), u, es) => reflectMirrored(Reflect(ObjectUnsafeImmutable(f(a))(mtype(e.mR)), mapOver(f,u), f(es)))(mtype(typ[A]), pos)
    case Reflect(e@ObjectUnsafeMutable(a), u, es) => reflectMirrored(Reflect(ObjectUnsafeMutable(f(a))(mtype(e.mR)), mapOver(f,u), f(es)))(mtype(typ[A]), pos)
...
trait ObjectOpsExpOpt extends ObjectOpsExp {
  override def object_tostring(lhs: Exp[Any])(implicit pos: SourceContext) = {
    if (lhs.tp <:< typ[String]) lhs.asInstanceOf[Exp[String]]
