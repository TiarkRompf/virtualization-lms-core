...
trait LiftVariables extends Base { this: Variables =>
...
trait ReadVarImplicit { this: Variables =>
  implicit def readVar[T:Typ](v: Var[T])(implicit pos: SourceContext) : Rep[T]
}

trait ReadVarImplicitExp extends BaseExp { this: VariablesExp =>
  implicit def readVar[T:Typ](v: Var[T])(implicit pos: SourceContext) : Exp[T] = toAtom(ReadVar(v))
}

trait LowPriorityVariableImplicits extends ImplicitOps { this: Variables =>
...
trait VariableImplicits extends LowPriorityVariableImplicits { this: Variables =>
...
trait VariablesExp extends Variables with PrimitiveOps with ImplicitOpsExp with VariableImplicits with ReadVarImplicitExp with AtomicWriteExp {
  // REMARK:
  ...
  case class NewVar[T:Typ](init: Exp[T]) extends Def2[T, Variable[T]]
  case class Assign[T:Typ](lhs: Var[T], rhs: Exp[T]) extends Def2[T, Unit]
  case class VarPlusEquals[T:Typ](lhs: Var[T], rhs: Exp[T]) extends Def2[T, Unit]
  case class VarMinusEquals[T:Typ](lhs: Var[T], rhs: Exp[T]) extends Def2[T, Unit]
  case class VarTimesEquals[T:Typ](lhs: Var[T], rhs: Exp[T]) extends Def2[T, Unit]
  case class VarDivideEquals[T:Typ](lhs: Var[T], rhs: Exp[T]) extends Def2[T, Unit]
  ...
  override def mirror[A:Typ](e: Def[A], f: Transformer)(implicit pos: SourceContext): Exp[A] = (e match {
    ...
    case Reflect(e@NewVar(a), u, es) => reflectMirrored(Reflect(NewVar(f(a))(e.mA), mapOver(f,u), f(es)))(mtype(typ[A]),pos)
    case Reflect(ReadVar(Variable(a)), u, es) => reflectMirrored(Reflect(ReadVar(Variable(f(a))), mapOver(f,u), f(es)))(mtype(typ[A]), pos)
    case Reflect(e@Assign(Variable(a),b), u, es) => reflectMirrored(Reflect(Assign(Variable(f(a)), f(b))(e.mA), mapOver(f,u), f(es)))(mtype(typ[A]), pos)
    case Reflect(e@VarPlusEquals(Variable(a),b), u, es) => reflectMirrored(Reflect(VarPlusEquals(Variable(f(a)), f(b))(e.mA), mapOver(f,u), f(es)))(mtype(typ[A]), pos)
    case Reflect(e@VarMinusEquals(Variable(a),b), u, es) => reflectMirrored(Reflect(VarMinusEquals(Variable(f(a)), f(b))(e.mA), mapOver(f,u), f(es)))(mtype(typ[A]), pos)
    case Reflect(e@VarTimesEquals(Variable(a),b), u, es) => reflectMirrored(Reflect(VarTimesEquals(Variable(f(a)), f(b))(e.mA), mapOver(f,u), f(es)))(mtype(typ[A]), pos)
    case Reflect(e@VarDivideEquals(Variable(a),b), u, es) => reflectMirrored(Reflect(VarDivideEquals(Variable(f(a)), f(b))(e.mA), mapOver(f,u), f(es)))(mtype(typ[A]), pos)
...
trait VariablesExpOpt extends VariablesExp {
  ...
  override implicit def readVar[T:Typ](v: Var[T])(implicit pos: SourceContext) : Exp[T] = {
    if (context ne null) {
      ...
      // if it is an assignment, just return the last value assigned
      val vs = v.e.asInstanceOf[Sym[Variable[T]]]
      //TODO: could use calculateDependencies(Read(v))

      val rhs = context.reverse.collectFirst {
        case w @ Def(Reflect(NewVar(rhs: Exp[_]), _, _)) if w == vs => Some(rhs.asInstanceOf[Exp[T]])
        case Def(Reflect(Assign(`v`, rhs: Exp[_]), _, _)) => Some(rhs.asInstanceOf[Exp[T]])
  ...
  override implicit def var_assign[T:Typ](v: Var[T], e: Exp[T])(implicit pos: SourceContext) : Exp[Unit] = {
    if (context ne null) {
      // find the last modification of variable v
      // if it is an assignment with the same value, we don't need to do anything
      val vs = v.e.asInstanceOf[Sym[Variable[T]]]
      //TODO: could use calculateDependencies(Read(v))

      context.reverse.foreach {
        case w @ Def(Reflect(NewVar(rhs), _, _)) if w == vs => if (rhs == e) return ()
        case Def(Reflect(Assign(`v`, rhs), _, _)) => if (rhs == e) return ()
...
trait ScalaGenVariables extends ScalaGenBase with ScalaGenImplicitOps {
  val IR: VariablesExp
  import IR._
...
trait CLikeGenVariables extends CLikeGenImplicitOps {
  val IR: VariablesExp
  import IR._
...
trait CudaGenVariables extends CudaGenBase with CLikeGenVariables
trait OpenCLGenVariables extends OpenCLGenBase with CLikeGenVariables
trait CGenVariables extends CGenBase with CLikeGenVariables
