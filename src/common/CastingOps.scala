...
trait CastingOps extends Variables with OverloadHack { this: ImplicitOps =>
  ...
  implicit def varAnyToCastingOps[A:Typ](lhs: Var[A])(implicit pos: SourceContext) = new CastingOpsCls(readVar(lhs))
...
trait CastingOpsExp extends CastingOps with BaseExp with BooleanOpsExp { this: ImplicitOps =>
  ...
  def rep_isinstanceof[A,B](lhs: Exp[A], mA: Typ[A], mB: Typ[B])(implicit pos: SourceContext) = toAtom(RepIsInstanceOf(lhs,mA,mB))
