...
trait ListBuffer extends Base with Expressions with Variables {
	...
	def newListBuffer[A:Manifest](): Rep[mutable.ListBuffer[A]]
...
trait ListBufferExp extends ListBuffer with BaseExp with EffectExp with Effects {
	...
	case class NewListBuffer[A:Manifest]() extends Def[mutable.ListBuffer[A]] {
		val m = manifest[A]
	}
	...
	def newListBuffer[A:Manifest]() = reflectEffect(NewListBuffer[A]())
...
trait ScalaGenListBuffer extends ScalaGenBase with ScalaNestedCodegen {
	...
	override def emitNode(sym: Sym[Any], rhs: Def[Any]) = rhs match {
		case l@NewListBuffer() =>
			emitValDef(sym, "new scala.collection.mutable.ListBuffer[" + remap(l.m) + "]")
