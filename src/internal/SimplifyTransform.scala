...
trait SimplifyTransform extends Scheduling { self =>
...
*/
  ...
  def transformOne[A](s: Sym[A], x: Def[A], t: SubstTransformer): Exp[A] = {
    ...
    val y = try {
      ...
      val tss = t(ss)
      if (ss != tss) {
        val s2 = mirror(x, t.asInstanceOf[Transformer])(mtype(s.tp), mpos(s.pos))
        if (s2 == s) {
          cerror("warning: mirroring of "+s+"="+x+" syms " + ss.mkString(",") + " returned same object (expected t(syms) = " + tss.mkString(",") + ")")
        }
          ...
          case Def(x2) =>
            if (x.getClass == x2.getClass) {
              ...
              if (ss2 != tss.filter(_.isInstanceOf[Sym[_]])) // should do filtering in def of tss above?
                cerror("warning: mirroring of "+s+"="+x+" syms " + ss.mkString(",") + " returned "+s2+"="+x2+" syms " + ss2.mkString(",") + " (expected t(syms) = " + tss.mkString(",") + ")")

            }
            if (!(s2.tp <:< s.tp))
              cerror("warning: mirroring of "+s+"="+x+" type " + s.tp + " returned "+s2+"="+x2+" type " + s2.tp + " (not a subtype)")

          case _ =>
            if (!(s2.tp <:< s.tp))
              cerror("warning: mirroring of "+s+"="+x+" type " + s.tp + " returned "+s2+" type " + s2.tp + " (not a subtype)")

        }
        s2
      } else {
        cdbg("skipping mirror operation "+s+"="+x+" syms " + ss.mkString(",") + " subst " + t.subst.mkString(","))
        s
      }
    } catch { //hack
      case e if e.toString contains "don't know how to mirror" =>
        ...
        s
      case e: Throwable =>
        cerror("error: exception during mirroring of "+x+": "+ e)
        e.printStackTrace;
        s
    }

    if (y != s) {
      if (y.isInstanceOf[Sym[_]] && findStm(y.asInstanceOf[Sym[Any]]).nonEmpty)
        cdbg("--> replace " + s+"="+x + " by " + y+"="+findStm(y.asInstanceOf[Sym[Any]]).get.defines(y.asInstanceOf[Sym[Any]]).get)
      else
        cdbg("--> replace " + s+"="+x + " by " + y)
  ...
  def transformLoopBody[A](s: Sym[A], x: Def[A], t: SubstTransformer): Def[A] = {
    ...
    implicit val pos: SourceContext = mpos(s.pos)
    mirrorDef(x, t)
  }
    ...
    scope flatMap {
      case TP(sym, rhs) =>
        transformOne(sym, rhs, t) match {
          case s: Sym[_] => (Option(scopeIndex.get(s)) orElse findStm(s)).toList // check scope before target graph
          case _ => Nil
        }
      case TTP(lhs, mhs, SimpleFatIfThenElse(c,as,bs)) =>
        // alternate strategy: transform thin def, then fatten again (a little detour)
        cdbg("need to transform rhs of fat if/then/else: " + lhs + ", if " + c + " then " + as + " else " + bs)
        val lhs2 = (lhs zip mhs).map { case (s,r) => transformOne(s, r, t) }.distinct.asInstanceOf[List[Sym[Any]]]
        val mhs2 = lhs2.map(s => findStm(s).get.defines(s).get)
        ...
        if (lhs != lhs2) {
          ...
          cdbg("lhs changed! will add to innerScope: "+missing.mkString(","))
          //innerScope = innerScope ::: missing
        }

        def toIf(d: Def[Any]) = d match {
          case l: AbstractIfThenElse[_] => l
          case Reflect(l: AbstractIfThenElse[_], _, _) => l
        }
        val cond2 = if (lhs != lhs2) mhs2.map (toIf(_).cond) reduceLeft { (s1,s2) => assert(s1==s2,"conditions don't agree: "+s1+","+s2); s1 }
                    else t(c)
        val as2 = (if (lhs != lhs2) (lhs2 zip (mhs2 map (toIf(_).thenp)))
                   else (lhs zip as)) map { case (s,r) => transformIfBody(s,r,t) }
        val bs2 = (if (lhs != lhs2) (lhs2 zip (mhs2 map (toIf(_).elsep)))
                   else (lhs zip bs)) map { case (s,r) => transformIfBody(s,r,t) }

        cdbg("came up with: " + lhs2 + ", if " + cond2 + " then " + as2 + " else " + bs2 + " with subst " + t.subst.mkString(","))
        List(TTP(lhs2, mhs2, SimpleFatIfThenElse(cond2,as2,bs2)))

      case TTP(lhs, mhs, SimpleFatLoop(s,x,rhs,tps)) =>
        // alternate strategy: transform thin def, then fatten again (a little detour)
        cdbg("need to transform rhs of fat loop: " + lhs + ", " + rhs)
        val lhs2 = (lhs zip mhs).map { case (s,r) => transformOne(s, r, t) }.distinct.asInstanceOf[List[Sym[Any]]]
        val mhs2 = lhs2.map(s => findStm(s).get.defines(s).get)
        if (lhs != lhs2) {
          val missing = (lhs2.map(s => findStm(s).get) diff scope/*innerScope*/)
          cdbg("lhs changed! will add to innerScope: "+missing.mkString(","))
          //innerScope = innerScope ::: missing
        }
        //val shape2 = if (lhs != lhs2) lhs2.map { case Def(SimpleLoop(s,_,_)) => s } reduceLeft { (s1,s2) => assert(s1==s2,"shapes don't agree: "+s1+","+s2); s1 }
        def toLoop(d: Def[Any]) = d match {
          case l: AbstractLoop[_] => l
          case Reflect(l: AbstractLoop[_], _, _) => l
        }
        val shape2 = if (lhs != lhs2) mhs2.map (toLoop(_).size) reduceLeft { (s1,s2) => assert(s1==s2,"shapes don't agree: "+s1+","+s2); s1 }
                     else t(s)
        val rhs2 = (if (lhs != lhs2) (lhs2 zip (mhs2 map (toLoop(_).body)))
  ...
  /*      //update innerScope -- change definition of lhs2 in place (necessary?)
        ...
        cdbg("came up with: " + lhs2 + ", " + rhs2 + " with subst " + t.subst.mkString(","))
        List(TTP(lhs2, mhs2, SimpleFatLoop(shape2,t(x).asInstanceOf[Sym[Int]],rhs2,lhs2.map(_.tp))))
  ...
  */
      ...
      case TTP(lhs, mhs, SimpleFatLoopNest(sizes,strides,vs,bodies,tps)) => Nil
  ...
  def transformAllFully(currentScope0: List[Stm], result0: List[Exp[Any]], t: SubstTransformer): (List[Stm], List[Exp[Any]]) = {
    ...
    def withEffectContext(body: =>List[Stm]): List[Stm] = {
      ...
      if (leftovereffects.nonEmpty)
        clog("warning: transformation left effect context (will be discarded): "+leftovereffects)
      context = save
      scope
    }
    ...
    if (currentScope != previousScope) { // check convergence
      cerror("error: transformation of scope contents has not converged")
      cdbg(previousScope + "-->" + currentScope)
    }
