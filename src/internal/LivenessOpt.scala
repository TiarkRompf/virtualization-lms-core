...
// TODO: Unused
trait LivenessOpt extends Traversal {
  import IR._
...
/*    case TP(sym, rhs) if used(sym) =>
  ...
  override def focusBlock[A](result: Block[Any])(body: => A): A = {
    super.focusBlock(result) {
      ...
      clog("gathering liveness info for block " + result)
...
// TODO: Unused
trait DefUseAnalysis extends Traversal {
  import IR._
  ...
  override def focusBlock[A](result: Block[Any])(body: => A): A = {
    super.focusBlock(result) {
      ...
      clog("gathering def-use info for block " + result)
