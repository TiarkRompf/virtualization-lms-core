...
/*
  current fusion algorithm:
    ...
    - build up a substitution to unify loop variables and replace a(i) expressions in fused bodies.
      for filter-style loops, create new loop statements (parent shape + added condition), add to substitution
...
trait LoopFusionOpt extends BlockTraversal with LoopFusionCore {
  ...
  override def focusExactScopeFat[A](resultB: List[Block[Any]])(body: List[Stm] => A): A = {
    ...
    if (result0 != result) {
      clog("super.focusExactScopeFat with result changed from " + result0 + " to " + result)

      (result0 zip result) foreach {
        case (r0 @ Def(Reify(x, _, _)),Def(Reify(y, u, es))) =>
          if (!x.isInstanceOf[Sym[Any]])
            clog("non-sym block result: " + x + " to " + y)
          else if (x != y)
...
trait LoopFusionCore extends Scheduling with CodeMotion with SimplifyTransform { self =>
  val IR: LoopsFatExp with IfThenElseFatExp
  import IR._
...
*/
    ...
    var Wloops: List[Stm] = {
      ...
      levelScope collect { case e @ TTP(_, _, SimpleFatLoop(_,_,_,_)) => e }
    }
    ...
    */
    ...
    if (Wloops.nonEmpty) {
      ...
      var UloopSyms = currentScope collect { case e @ TTP(lhs, _, SimpleFatLoop(_,_,_,_)) if !Wloops.contains(e) => lhs }

      //do{

        // utils
        def WgetLoopShape(e: Stm): Exp[Int] = e.rhs match { case SimpleFatLoop(s,x,rhs,tps) => s }
        def WgetLoopVar(e: Stm): List[Sym[Int]] = e.rhs match { case SimpleFatLoop(s,x,rhs,tps) => List(x) }
        def WgetLoopRes(e: Stm): List[Def[Any]] = e.rhs match { case SimpleFatLoop(s,x,rhs,tps) => rhs }
        ...
        var WtableNeg = Wloops.flatMap { dx => // find non-simple dependencies (other than a(i))
          ...
          getSchedule(currentScope)(WgetLoopRes(dx)) flatMap {
            ...
            case sc =>
              ...
              if (pr.nonEmpty) clog("fusion of "+pr+" prevented by " + sc + " which is required by body of " + dx.lhs)// + " / " + thisLoopVars)
              pr
          }
        }.distinct
        ...
        clog("wtableneg: " + WtableNeg) // will add more later, need to maintain closure
        ...
        def canFuseDirect(a: Stm, b: Stm): Boolean = (a.rhs,b.rhs) match {
          ...
          case (SimpleFatLoop(Def(SimpleDomain(a1)),_,arhs,_), SimpleFatLoop(_,_,brhs,_)) if (b.lhs contains a1) && arhs.forall(canApplyAddCondition) => true // pipeline
          case (SimpleFatLoop(_,_,arhs,_), SimpleFatLoop(Def(SimpleDomain(b1)),_,brhs,_)) if (a.lhs contains b1) && brhs.forall(canApplyAddCondition) => true
          case _ => false
        }
        ...
        def extendLoopWithCondition(e: Stm, shape: Exp[Int], targetVar: Sym[Int], c: List[Exp[Boolean]]): List[Exp[Any]] = e.rhs match {
          case SimpleFatLoop(s,x,rhs,tps) => (e.lhs zip rhs).map { case (l,r) =>
            findOrCreateDefinitionExp(SimpleLoop(shape,targetVar,applyAddCondition(r,c)))(mtype(l.tp), mpos(l.pos)) }
        }
        ...
        for (b@ TTP(_,_,_) <- partitionsIn) {
          ...
          partitionsOut.find(a => canFuse(a,b)) match {
            case Some(a: TTP) =>
              ...
              val fused = TTP(lhs, a.mhs ++ b.mhs, SimpleFatLoop(shape, targetVar, WgetLoopRes(a):::WgetLoopRes(b), a.lhs.map(_.tp):::b.lhs.map(_.tp) ))
        ...
        if ((partitionsOut intersect partitionsIn) != partitionsOut) { // was there any change?
            ...
            case e@TP(s, SimpleIndex(a, i)) => // if a is a loop result, i is a loop var, and both loops are fused
              ...
              partitionsOut.find(_.lhs contains a) match {
                case Some(fused) if WgetLoopVar(fused) contains t(i) =>
                  ...
                  clog("replace " + e + " at " + index + " within " + fused)
          ...
          Wloops = pOutT map {
            case TTP(lhs, mhs, SimpleFatLoop(s, x, rhs,tps)) =>
              ...
              TTP(select(lhs, ex), select(mhs, ex), SimpleFatLoop(s, x, select(rhs, ex), select(lhs,ex).map(_.tp) ))
          }
