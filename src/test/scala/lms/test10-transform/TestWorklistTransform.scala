...
trait FWTransform1 extends BaseFatExp with EffectExp with IfThenElseFatExp with LoopsFatExp { self =>
  ...
  protected override implicit def toAtom[A:TypeRep](d: Def[A])(implicit pos: SourceContext): Exp[A] = { // override createDefinition instead?
    val in = syms(d)
    val actual = xform(in)
...
/*
  override def vlength[T:TypeRep](a: Rep[Vector[T]]) = a match {
  ...
  def vfromarray[A:TypeRep](x: Exp[Array[A]]): Exp[Vector[A]] = VectorFromArray(x)
  def vtoarray[A:TypeRep](x: Exp[Vector[A]]): Exp[Array[A]] = x match {
...
*/
  ...
  override def vlength[T:TypeRep](a: Rep[Vector[T]]) = {
