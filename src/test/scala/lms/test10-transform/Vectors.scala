...
trait VectorOps extends Base {
  ...
  def vliteral[T:TypeRep](a: List[Rep[T]]): Rep[Vector[T]]
  def vapply[T:TypeRep](a: Rep[Vector[T]], x: Rep[Int]): Rep[T]
  def vupdate[T:TypeRep](a: Rep[Vector[T]], x: Rep[Int], y: Rep[T]): Rep[Unit]
  def vlength[T:TypeRep](a: Rep[Vector[T]]): Rep[Int]
...
trait VectorExp extends VectorOps with EffectExp {
  ...
  def vliteral[T:TypeRep](a: List[Rep[T]]): Rep[Vector[T]] = VectorLiteral(a)
  def vplus(a: Rep[Vector[Double]], b: Rep[Vector[Double]]): Rep[Vector[Double]] = VectorPlus(a,b)
  def vapply[T:TypeRep](a: Rep[Vector[T]], x: Rep[Int]): Rep[T] = VectorApply(a,x)
  def vupdate[T:TypeRep](a: Rep[Vector[T]], x: Rep[Int], y: Rep[T]): Rep[Unit] = VectorUpdate(a,x,y)
  def vlength[T:TypeRep](a: Rep[Vector[T]]): Rep[Int] = VectorLength(a)
  ...
  override def mirror[A:TypeRep](e: Def[A], f: Transformer)(implicit pos: SourceContext): Exp[A] = (e match {
    ...
    case VectorApply(a,x) => vapply(f(a),f(x))(mtype(typeRep[A]))
