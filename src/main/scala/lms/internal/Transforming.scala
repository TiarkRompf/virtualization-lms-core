...
trait AbstractTransformer {
  ...
  def apply[A:TypeRep](xs: Block[A]): Block[A] = {
...
trait Transforming extends Expressions with Blocks with OverloadHack {
  ...
  def mtype[A,B](m:TypeRep[A]): TypeRep[B] = m.asInstanceOf[TypeRep[B]] // hack: need to pass explicit manifest during mirroring
  ...
  def mirror[A:TypeRep](e: Def[A], f: Transformer)(implicit pos: SourceContext): Exp[A] = mirrorDef(e,f)

  def mirrorDef[A:TypeRep](e: Def[A], f: Transformer)(implicit pos: SourceContext): Def[A] = sys.error("don't know how to mirror " + e)

  def mirrorFatDef[A:TypeRep](e: Def[A], f: Transformer)(implicit pos: SourceContext): Def[A] = sys.error("don't know how to mirror " + e) //hm...
...
trait FatTransforming extends Transforming with FatExpressions {
  ...
  override def mirror[A:TypeRep](e: Def[A], f: Transformer)(implicit pos: SourceContext): Exp[A] = (e match {
    case Forward(x) => toAtom(Forward(f(x)))(mtype(typeRep[A]),pos)
    case _ => super.mirror(e,f)
  }).asInstanceOf[Exp[A]]

  //def mirror[A:TypeRep](e: FatDef, f: Transformer): Exp[A] = sys.error("don't know how to mirror " + e)
