...
trait StructExp extends Structs with BaseExp with EffectExp {
  ...
  case class Field[T](struct: Rep[Any], index: String, tp: TypeRep[T]) extends Def[T]

  def struct[T:TypeRep](tag: StructTag[T], elems: (String, Rep[Any])*)(implicit pos: SourceContext): Rep[T] = struct(tag, Map(elems:_*))
  def struct[T:TypeRep](tag: StructTag[T], elems: Map[String, Rep[Any]])(implicit pos: SourceContext): Rep[T] = SimpleStruct[T](tag, elems)

  def field[T:TypeRep](struct: Rep[Any], index: String)(implicit pos: SourceContext): Rep[T] = Field[T](struct, index, typeRep[T])

  override def record_new[T :TypeRep](fields: Seq[(String, Boolean, Rep[T] => Rep[_])]) = {
    val r = fresh[T]
  ...
  override def record_select[T :TypeRep](record: Rep[Record], fieldName: String) = {
    field(record, fieldName)
  }

  //FIXME: reflectMutable has to take the Def
  //def mfield[T:TypeRep](struct: Rep[Any], index: String): Rep[T] = reflectMutable(Field[T](struct, index, typeRep[T]))
...
trait StructExpOpt extends StructExp {
  ...
  override def field[T:TypeRep](struct: Rep[Any], index: String)(implicit pos: SourceContext): Rep[T] = struct match {
...
trait StructExpOptCommon extends StructExpOpt with VariablesExp with IfThenElseExp {
  ...
  override def readVar[T:TypeRep](v: Var[T])(implicit pos: SourceContext): Exp[T] = v match {
    case Variable(Def(Struct(NestClassTag(tag), elems: Map[String,Exp[Variable[Any]]]))) =>
      def unwrap[A](m:TypeRep[Variable[A]]): TypeRep[A] = m.typeArguments match {
        case a::_ => mtype(typeRepFromManifest(a))
  ...
  /*def reReify[T:TypeRep](a: Rep[T]): Rep[T] = a match { // TODO: should work with loop bodies, too (Def!)
  ...
  override def ifThenElse[T:TypeRep](cond: Rep[Boolean], a: Rep[T], b: Rep[T]) = (reReify(a),reReify(b)) match {
...
trait ScalaGenStruct extends ScalaGenBase {
  ...
  override def emitNode(sym: Sym[Any], rhs: Def[Any]) = rhs match {
    case Struct(tag, elems) =>
      /* TODO: emit code that creates an object corresponding to the tag and the typerep
  ...
  override def remap[A](m: TypeRep[A]) = m match {
    case m if m <:< typeRep[Record] => recordClassName(m)
    case _ => super.remap(m)
  }

  // not public because should not be called with a typerep not describing a subtype of Manifest[Record]
  protected def recordClassName[A](m: TypeRep[A]): String = m.mf match {
    case rm: reflect.RefinedManifest[A] => rm.runtimeClass.getSimpleName + rm.fields.map(f => recordClassName(f._2)).mkString
    case _ => m.runtimeClass.getSimpleName + m.typeArguments.map(a => recordClassName(a)).mkString
  }

  private val encounteredStructs = collection.mutable.HashMap.empty[String, Map[String, Exp[_]]]
  private def registerType[A](m: TypeRep[A], fields: Map[String, Exp[_]]) {
