...
trait TupledFunctionsExp extends TupledFunctions with FunctionsExp with TupleOpsExp {
  ...
  private def tupledTypeRep[T](m:TypeRep[T]): Boolean = m.erasure.getName startsWith "scala.Tuple"
  private def tupledTypeRepOf[T](m:TypeRep[T], arity: Int): Boolean = m.erasure.getName == "scala.Tuple" + arity
...
trait ScalaGenTupledFunctions extends ScalaGenFunctions with GenericGenUnboxedTupleAccess {
  ...
  def unwrapTupleStr[A](m:TypeRep[A]): Array[String] = {
    val s = m.toString
