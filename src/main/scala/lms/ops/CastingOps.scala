...
trait CastingOps extends Variables with OverloadHack {
  ...
  def rep_asinstanceof[A,B](lhs: Rep[A], mA: TypeRep[A], mB: TypeRep[B])(implicit pos: SourceContext) : Rep[B]
}
...
trait CastingOpsExp extends CastingOps with BaseExp {
  ...
  case class RepAsInstanceOf[A,B](lhs: Exp[A], mA: TypeRep[A], mB: TypeRep[B]) extends Def[B]

  def rep_isinstanceof[A,B](lhs: Exp[A], mA: TypeRep[A], mB: TypeRep[B])(implicit pos: SourceContext) = RepIsInstanceOf(lhs,mA,mB)
  def rep_asinstanceof[A,B](lhs: Exp[A], mA: TypeRep[A], mB: TypeRep[B])(implicit pos: SourceContext) : Exp[B] = toAtom(RepAsInstanceOf(lhs,mA,mB))(mB,pos)
