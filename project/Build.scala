...
object LMSBuild extends Build {
  ...
  lazy val virtScala = Option(System.getenv("SCALA_VIRTUALIZED_VERSION")).getOrElse("2.10.0")
  ...
  lazy val docs = Project(
    id = "docs",
    base = file("./docs"),
    settings = Project.defaultSettings ++ buildSettings ++ virtSettings ++ continuationsSettings ++ siteSettings ++ docsSettings
  ) dependsOn (lms)
