...
trait Matching extends Base {
  ...
  def evalMatch[A,B](x: Rep[A], alts: List[PartialFunction[Rep[A],Rep[B]]]): Rep[B]
  
  implicit def switchable[A](x:Rep[A]) = new {
    def switch[B](f: PartialFunction[Rep[A],Rep[B]]) = new Match[A,B](x, List(f))
  }
  
  class Match[A,B](x: Rep[A], alts: List[PartialFunction[Rep[A],Rep[B]]]) {
  ...
  def _match[A,B](x: Rep[A])(cases: PartialFunction[Rep[A],Rep[B]]*): Rep[B] = {
    evalMatch(x, List())
  }
}
...
trait MatchingExtractorExp extends FunctionExp with Effects with Control {
...
trait MatchingExtractorExpOpt extends MatchingExtractorExp {
...
/*
trait MatchingExtractorExpOpt2 extends MatchingExtractorExpOpt {
